...
\item
	...
	\item[Variante 1:]
		...
		Dies ist die Darstellungsmatrix von $\pi_{1*}$ bez"uglich den Basen $\pdifffrac{}{\phi_1^1}, \ldots, \pdifffrac{}{\phi_1^d}$ und $\pdifffrac{}{(\phi_1 \times \phi_2)^1}, \ldots, \pdifffrac{}{(\phi_1 \times \phi_2)^{\ddim M_1 + \ddim M_2}}$. Also ist $\pi_{1*}$ surjektiv. Auch $\pi_1$ ist surjektiv, also ist $\pi_1$ eine Submersion. Der Beweis f"ur $\pi_2$ folgt analog.
	\item[Variante 2:]
		Sei $X = \sum_{j = 0}^{\ddim M_1} \xi^j \pdifffrac[p_1]{}{\phi_1^j} \in \T_{p_1}M_1$. Setze
			\[ \tilde X = \sum_{j=1}^{\ddim M_1} \xi^j \pdifffrac[p]{}{(\phi_1 \times \phi_2)^j} \in \T_p(M_1 \times M_2). \]
		Dann ist
		\begin{align*}
			\pi_{1*_p} \tilde X &= \sum_{k=1}^{\ddim M_1} \left( \sum_j \smash{\underbrace{\partial_j \left( \phi^k \circ \pi_1 \circ (\phi_1 \times \phi_2)^{-1} \right)}_{= \delta(k,j)}} \left( \phi_1(p_1), \phi_2(p_2) \right) \xi^j \right) \pdifffrac[p_1]{}{\phi_1^k} \\
			&= \sum_{k=1}^{\ddim M_1} \xi^k \pdifffrac[p_1]{}{\phi_1^k} = X
		\end{align*}
