...
\begin{center}\begin{tikzpicture}[font=\scriptsize]
	...
	\path[name path=circle,draw] (0,0) circle (\radius); \draw[dashed] (-\radius,0) to[out=\angle,in=180-\angle] (\radius,0); \draw (-\radius,0) to[out=360-\angle,in=180+\angle] (\radius,0);
	...
	\path[name path=upper,draw] (N) node[above]{$N$} -- (Pkt1)node[left]{$(0,\varphi(p))$}; \fill (N) circle (0.05) (Pkt1) circle (0.05);
	...
	\draw (S) -- ($(S)+2.5*(direction2)$); \fill ($(S)+1.25*(direction2)$) circle(0.05) node[right]{$(0, \varphi(p))$};
...
\item
	...
	f"ur $y \in \R^n \setminus \{0\}$. Also glatter Kartenwechsel.\marginnote{\begin{tikzpicture}[font=\scriptsize]
		...
		\def\anglePoint{65}	
		\path[name path=circle,draw] (0,0) circle (\radius); \draw[dashed] (-\radius,0) to[out=\angle,in=180-\angle] (\radius,0); \draw (-\radius,0) to[out=360-\angle,in=180+\angle] (\radius,0);
		\draw[->] (\anglePoint:\radius) node[anchor=south west]{$p$} -- ($(\anglePoint:\radius)+(270:1.2)$); \fill (\anglePoint:\radius) circle(0.05) ($(\anglePoint:\radius)+(270:1.25)$) circle(0.05) node[anchor=north]{$\varphi_i^+(p)$};
	\end{tikzpicture}}
