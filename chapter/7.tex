...
\begin{align*}
  \ric_p(X,Y) = \operatorname{spur} R(\cdot,X)Y = \sum_i\left<R(e_i,X)Y,e_i\right>
% spur = trace (tr)
\end{align*}
heißt \CmMark{Ricci-Tensor} in $p$.

Aus den Symmetrien von $R$ folgt, dass $\ric_p$ symmetrisch ist.
...
\begin{Dfn}
  ...
  \end{align*}
die \CmMark{Ricci-Krümmung} in $p$ in Richtung $X$.
\end{Dfn}

Die Spur von $\Ric$
\begin{align*}
  \operatorname{scal}(p) = \operatorname{spur} ... \text{\textcolor{red}{Hier fehlt noch etwas!}}
\end{align*}
