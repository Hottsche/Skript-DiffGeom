...
\begin{dfn*}
  Eine $n$-dimensionale \CmMark{topologische Mannigfaltigkeit} $M$ ist ein topologischer Hausdorff-Raum mit einer abzählbaren Basis der Topologie in dem zu jedem Punkt $p \in M$ eine offene Menge $U$ mit $p \in U$ existiert und ein Homöomorphismus
  \begin{align*}
    \phi \colon U \to V
  \end{align*}
  auf eine offene Menge $V \subset \R^{n}$.
\end{dfn*}

% Bem: Satz von Brouwer: Sind zwei Mengen im $\R^n$ und $\R^{m}$ homöomorph, dann gilt schon $n = m$. (Schwer zu beweisen.)
...
\begin{dfn*}
  ...
  Die Abbildung $\varphi \colon U \to V \subset \R^n$ heißt \CmMark{Karte} von $M$ um $p$, die Menge $U$ hei0t Kartengebiet.\\
...
Die Menge aller glatten Abbildungen von $M$ nach $N$ wied $C^{\infty}(M,N)$ genannt.

\end{dfn}

\textbf{Konvention}: Ab jetzt sind alle Mannigfaltigkeiten, wie auch alle Abbildungen glatt.
...
\begin{bsp}
  \begin{enumerate}
  \item $(\varphi,U) \in \mathcal A \to \varphi \in C^{\infty}(U,\R^n)$, $\Id_{\R^n}\circ \varphi \circ \varphi^{-1} = \varphi \circ \varphi^{-1} \in C^{\infty}$.
