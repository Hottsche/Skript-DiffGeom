...
Seien $\mathcal A = \{(\varphi_{\alpha}, U_{\alpha} \mid \alpha \in J \}$ und $\mathcal A' = \{(\psi_{\beta}, U_{\beta}) \mid \beta \in I\}$ $C^{\infty}$ Atlanten von $M$ und $N$. Dann ist $\mathcal B = \{(\varphi_{\alpha}\times\psi_{\beta},U_{\alpha}\times U_{\beta} \mid (\alpha, \beta) \in I \times J\}$ ein $C^{\infty}$-Atlas von $M\times N$, denn 
\begin{align*}
  (\varphi_{\alpha} \times \psi_{\beta}) \circ (\varphi_{\mu} \times \psi_{\nu})^{-1} = (\varphi_{\alpha} \circ \varphi_{nu}^{-1}) \times (\psi_{\beta} \circ \psi_{nu}^{-1})
\end{align*}
...
\begin{lemma}
  Es seien $N$ eine $n$-dimensionale glatte Mannigfaltigkeit und $M \subset N$ eine $m$-dimensionale Untermannigfaltigkeit von $M$. Bezeichnet $\mathcal A$ einen $C^{\infty}$-Atlas von $N$ und $\pi \colon \R^n \to \R^m, (x^1, \ldots, x^m,\ldots,x^n) \mapsto (x^1, \ldots, x^m)$, so ist
  \begin{align*}
    \mathcal B = \{(\pi \circ \varphi|_{U \cap M},U\cap M) mid (\varphi, U) in \mathcal A \text{ an M adaptierte Karte}\}
  \end{align*}
...
\begin{proof}
  ...
  Ist $p \in N$, so existiert eine adaptierte Karte $(\varphi,U)$ von $N$ um $p$ und $\pi \circ \varpi|_{U \cap M}$ ist ein Homöomorphismus von $U \cap M$ auf eine offene Teilmenge des $\R^m$.\\
  Jeder Kartenwechsel
  \begin{align*}
