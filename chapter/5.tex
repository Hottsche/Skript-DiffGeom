...
\begin{Dfn}
  ...
  \item Es existiert eine Umgebung $U$ von $p$ in $M$ und ein Diffeomorphismus
    ...
    Ein solches $\tau$ heißt \CmMark[Karte!B\"undel-]{B"undelkarte}.
  \end{enumerate}
\end{Dfn}
...
Es seien $V$ und $W$ $k$-Vektorr"aume. Das \CmMark[Tensor!-produkt]{Tensorprodukt} $V \otimes W$ ist der von den Elementen $v \otimes w$ mit $v \in V$, $w \in W$ und den Relationen \begin{enumerate}[label=(\roman*),widest=iii]
\item
...
\begin{emptythm}[Universelle Eigenschaft:]
Ist $U$ ein Vektorraum und $\sigma$ eine schiefsymmetrische multilineare Abbildung $\underbrace{V \X \ldots  \X V}_{p\text{-mal}} \to U$, so existiert genau eine lineare Abbildung $\phi: \Lambda^pV \to U$ mit $\sigma = \phi \circ s$.
\begin{center}\begin{tikzpicture}
...
induzierten Produkt zu einer assoziativen, graduiert kommutativen Algebra: $v \in \Lambda^pV$, $w \in \Lambda^qV$, $v \wedge w = (-1)^{p \cdot q} w \wedge v$. Sind $V_1$, $V_2$, $W_1$, $W_2$ Vektorr"aume und $\phi \in \Hom(V_1, W_1)$, $\psi \in \Hom(V_2, W_2)$, so definiert die Fortsetzung von 
...
ein Element $\phi \otimes \psi \in \Hom(V_1 \otimes V_2, W_1 \otimes W_2)$. Sind $V$, $W$ Vektorr"aume und $\phi_1,\ldots ,\phi_p \in \Hom(V, W)$, so induzieren diese eine lineare Abbildung
	\[ \phi_1 \otimes \ldots \otimes \phi_p : \bigoplus^p V \to \bigoplus^p W \]
und
	\[ \phi_1 \wedge \ldots  \wedge \phi_p : \Lambda^pV \to \Lambda^pW \]
...
Die Schnitte des $(r,s)$-Tensorbündels $\Gamma(\T_s^r(\T M)) = \mathcal T_s^r(M)$ bezeichnet man als $(r,s)$-\CmMark[Tensor!-feld]{Tensorfelder} auf $M$. Die $(1,0)$-Tensorfelder sind genau die Vektorfelder auf $M$; $\mathcal T_0^1(M) = \mathcal V(M)$. Es bezeichne $\mathcal V^{*}(M) = \mathcal T_1^0(M)$ den Raum der $(0,1)$-Tensorfelder.
...
\end{align*}
Die $\dop x^i$ sind also punktweise linear unabhängige $(0,1)$-Tensorfelder über dem Kartengebiet $U$.
...
denn
	\[ \alpha_j^i = \textcolor{red}{\pdifffrac{}{y^j}(\phi^i)} = \pdifffrac{p^i}{y^i} = \partial_j(\phi^i\circ \psi^{-1}) \]
...
\end{align*}
Diese transponierten Inversen der Differentiale der Kartenwechsel sind genau die Übergangsfunktionen $h_{\alpha\beta} = (g_{\alpha\beta}^{*})^{-1}$ in der Definition von $(\T M)^{*} = \T^{*}M$.
...
\begin{Dfn}\label{def-5-5}
  Es sei $X \in \mathcal V(M)$ mit dem Fluss $\gamma$ und $S \in \mathcal T_s^r(M)$ ein glattes Tensorfeld auf $M$. Dann heißt
  \begin{align*}
...
% Definition 5.9
\begin{Dfn}
  Es sei $\omega \in \Omega^k(M)$. Die $(k+1)$-Form $\dop \omega$ heißt das \CmMark[Differential]{(äußere) Differential}\index{Differential!\"au\ss eres} der Form $\omega$.
