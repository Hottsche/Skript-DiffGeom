...
Die Schnitte des $(r,s)$-Tensorbündels $\Gamma(\T_s^r(\T M)) = \mathcal T_s^r(M)$ bezeichnet man als \CmMark{$(r,s)$-Tensorfelder} auf $M$.
...
\begin{Prop}
  ...
  \begin{align*}
    & \underbrace{\mathcal V^{*}(M) \X \cdots \X \mathcal V^{*}(M)}_{r-\text{mal}} \X \underbrace{\mathcal V(M) \X \cdots \X \mathcal V(M)}_{k-\text{mal}} \to C^{\infty}(M),\\
...
\begin{bsp}
  ...
  \item Die Lieklammer $[\cdot,\cdot]$ ist nicht $C^{\infty}(M)$-linear, also \textbf{kein} Tensorfeld.
  \end{enumerate}
\end{bsp}

Ein Element von $\T_p^kM$ bezeichnet man als \CmMark[Kotangentialvektor]{Kotangentialvektoren}, das Bündel $\T^{*}M$ als \CmMark{Kotangentialbündel}.
...
Ist $\psi$ eine weitere Karte und bezeichnen $\pdifffrac{}{y^i}$ bzw. $\dop y^i$ die entsprechenden Koordinaten(ko)tangentialvektoren, so gilt:
\begin{align*}
  \dop x^i & = \sum \alpha_j^i\dop y^j \text{ mit } \alpha_j^i \overset{!}{=} \dop x^i\left(\pdifffrac{}{y^j}\right) = \sum \alpha_k^i \underbrace{\dop y^k\left(\pdifffrac{}{y^j}\right)}_{\delta_j^k}, \text{ denn }\\
  \alpha_j^i & = \dop x^i \left(\pdifffrac{}{y^j}\right) - \pdifffrac{}{y^j}(\varphi^i) = \pdifffrac{p^i}{y^i} = \partial_j(\varphi^i\circ \psi^{-1}).
\end{align*}
Es gilt also $\alpha = \D(\psi \circ \varphi^{-1})^{x^{-1}}$ vergleiche Satz 2.10
\begin{align*}
...
\textcolor{red}{Abbildung 10.3 Diagramm Tensorfeld auf N und pullback}
\begin{align*}
...
Es gilt $\Phi^{*}(S \otimes T) = \Phi^{*}S \otimes \Phi^{*}T$ für $S \in \mathcal T_1^0(N), T \in \mathcal T_t^0(N)$.
...
& \Phi^{*}S \colon \mathcal V^{*}M \times V\mathcal V(M)\\
...
\end{align*}
Insbesondere $X \in \mathcal T_0^1(M) = \mathcal V(M)$:
\begin{align*}
...
\begin{bsp}[Anwendung]
  Es sei $(\varphi_{\alpha},U_{\alpha})$ ein Atlas von $M$. Dann ist jedes $\varphi_{\alpha}$ ein Diffeomorphismus von $U_{\alpha}$ auf $\varphi_{\alpha}(U_{\alpha}) = V_{\alpha} \subset \R^m$.

  Ist $S \in \mathcal T_s^r(M)$, so ist
  \begin{align*}
  ...
  \begin{align*}
    ...
    & = \left.(\varphi_{\beta}^{-1})^{*} \circ \varphi_{\alpha}^{*} \circ (\varphi_{\alpha}^{-1})^{*} S|_{U_{\alpha}}\right|_{V_{\alpha} \cap V_{\beta}} = (\varphi_{\beta}^{-1})^{*}S|_{U_{\beta}}|_{V_{\alpha} \cap V_{\beta}} = S_{\beta}|_{V_{\alpha} \cap V_{\beta}}
  \end{align*}
...
\begin{Dfn}
  Es sei $X \in \mathcal V(M)$ mit dem Fluss $\gamma$ und $S \in \mathcal T_s^r$ ein glattes Vektorfeld auf $M$. Dann heißt
  \begin{align*}
...
\begin{bem}[Eigenschaften]
  ...
  \item $X,Y \in \mathcal T_0^1(M) = \mathcal V(M)$, so gilt
    \begin{align*}
      \mathcal L_XY = [X,Y].
    \end{align*}
\item Für $S \in \mathcal T_s^r(M), T \in \mathcal T_s^{r'}(M)$ gilt:
  \begin{align*}
...
Desweiteren sind (lokal) die $k$-Formen $\dop x^{i_1} \wedge \ldots \wedge \dop x^{i_k}$ mit $i_1 < \cdots < i_k$ eine Basis von $\Omega^k$. Jede $k$-Form $\omega$ ist lokal von der Gestalt
\begin{align*}
  \omega = \sum_{i_1 < \cdots y i_k}f_{i_1,\ldots,i_k}\dop x^{i_1} \wedge \cdots \wedge \dop x^{i_k}.
\end{align*}
\end{Dfn}
