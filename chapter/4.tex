...
\begin{align*}
  \gamma_{*\gamma^t(p)}^{-t}(Y_{\gamma^t(p)})=(\gamma_{*}^{-t}Y)_p \in \T_pM.
\end{align*}
Die Differenz $(\gamma_{*\gamma^t(p)}^{-t}Y)_p - Y_p$ ist also wohldefiniert. 
%\end{Bem}
...
Das durch 
\begin{align*}
  p \mapsto \lim_{t\to 0}\frac{1}t\left(\left(\gamma_{*\gamma^t(p)}^{-t}Y\right)_p-Y_p\right)
\end{align*}
