...
Dann gilt:
\begin{align*}
  tg_t(q) = \int_0^1h'(ts,q)(t)ds = \int_0^th' - h(t,q)' - h(0,q) - f(\textcolor{red}{\text{hier fehlt etwas!}}
\end{align*}
...
\begin{proof}
  Es seien $X,Y, Z \in \mahtcal V(M)$ und $\gamma$ der Fluss von $X$.
  Dann gilt:
  \begin{align*}
    [X,[Y,Z]] & = \mathcal L_X[Y,Z] 
    = \difffrac[t=0]{}{t}\left(\gamma_{\ast}^{-1}[Y,Z]\right)\\
    & = \difffrac[t=0]{}{t}[\gamma_{\ast}^{-t}Y,\gamma_{\ast}^{-t}Z]\\
    & = [\difffrac[t=0]{}{t} \gamma_{\ast}^{-t}Y,Z] + [Y, \difffrac[t=0]{}{t}\gamma_{\ast}^{-t}Z]\\
