...
Eine \quot{Kurve} durch $\gamma$ ist eine sogenannte \CmMark[Variation|glatte]{glatte Variation} $h\colon[0,1]\times[0,1] \to M, h(s,t) = h_s(t)$ mit $h(0) = \gamma$ und $h_s(0) = p$, sowie $h_s(1) = q, \ \forall s \in [0,1]$.
...
% Definition 8.1
\begin{Dfn}
  Eine glatte Kurve $c$ in $M$ heißt \CmMark{Geodätische}\footnote{Die Äquivalenz zur bereits bekannten Definition wird in Kürze gezeigt.}, wenn $\nabla_t\dot c \cong 0$ gilt.
\end{Dfn}

Ist $c$ Geodätische, so ist $c$ proportional zur Bogenlängenparametrisierung, d.h. $\|\dot c\| = $const, denn $\difffrac{}{t}\|\dot c(t)\|^{2} = \difffrac{}{t}\left<\dot c(t),\dot c(t) \right> = 2\left<\nabla_t\dot c(t), \dot c(t)\right> = 0$ .
...
\begin{bew}
  ...
  \item (Alternativ)
    ...
    Der maximale Fluss $g^t$ von $S$ heißt \CmMark[Fluss|geodätischer]{geodätischer Fluss}.
