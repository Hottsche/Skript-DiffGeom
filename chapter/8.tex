...
%	\draw[step=0.25,gray!15] (-3,-6) grid (9,6); \draw[step=0.5,gray!30] (-3,-6) grid (9,6); \fill (0,0) circle(0.1); %Hilfsgitter
	...
	%\draw[->] ($(0p) + (1.25,0)$) arc (0:25:1.25) node[right]{$\vartheta$};
	\draw[->] ($(0p) + (1.25,0)$) arc (0:12.5:1.25) node[right]{$\vartheta$} arc(12.5:25:1.25);
	...
	\node (q) at (5,0) {$q= \exp_p(v)$}; \node (v) at (5,3.5) {$v$}; \node[align=flush left,text depth=-18pt,anchor=west] (v2) at (7.4,3.5) {$(\| v \|, \vartheta)$\\ $= (r,\vartheta)$ \\ $=\varphi(q) \in (0,\varepsilon) \X S^1$}; \node[anchor=west] (R) at (7.4,4) {$\R^2$};
	\draw[|->] (q) -- (v); \draw[|->] (v) -- (v2);
	\draw[->] ($(v) + (0,0.5)$) -- (R);
\end{tikzpicture}
\end{center}
