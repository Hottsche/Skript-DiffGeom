...
\begin{bew}
  ...
  Sei weiter $c \colon [0,1]$ eine beliebige glatte Kurve von $p$ nach $q$ mit Koordinaten $\phi(c(t)) = (r(t), \theta^1(t), \ldots, \theta^{m-1}(t))$.
  \begin{center}\begin{tikzpicture}[font=\scriptsize]
...
\begin{bew}
  ...
  F"ur $p \in ;$ existiert nach Satz \ref{satz-8-7} ein $\epsilon > 0$, so dass $\overline \B_\epsilon(p)$ kompakt ist.
...
\begin{Satz}[Satz von Hopf-Rinow]\label{satz-8-11}\label{hopf-rinow}
  ...
  \item F"ur alle $p \in M$ gilt $\calD_p = \T_pM$, also $\exp$ ist auf ganz $M$
...
\begin{bew}
  Man zeigt zun"achst, dass falls (iii) f"ur $p \in M$ gilt, es zu jedem  $q \in M$ eine minimierende Geod"atische von $p$ nach $q$ gibt.
