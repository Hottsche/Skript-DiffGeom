...
%	\tikzgitter{(-5,-5)}{(5,5)} % Hilfsgitter
    ...
    \foreach \y in {1, 2}{
		...
		\draw[dashed] (p) ..controls(p) and ($(coord)-1*(ctrl)$).. (coord) ..controls($(coord) + (ctrl)$) and (q).. (q); } \foreach \y in {1, 2, 3}{ \path (a) ..controls(a) and ($-1*(ctrl2)$).. coordinate[pos=1 - \y * \faktor] (coord) (0,0) ..controls(ctrl2) and (b).. (b);
		% \draw[red] (coord) circle(0.05);
		\draw[dashed] (p) ..controls(p) and ($(coord)-1*(ctrl)$).. (coord) ..controls($(coord) + (ctrl)$) and
		(q).. (q);
	}
...
\begin{center}\textcolor{red}{[BILD, ich werde es irgenwann mal zuende zeichnen, es ist auf den ersten Seiten des Buches zu finden, wer es sehen will]}\\
\begin{tikzpicture}
	\tikzgitter{(-6,0)}{(6,15)}
	
	% die grosse Ellipse (copy-paste von oben)
	\def\hoehe{2}
	\def\breite{1.25}
	\coordinate (mitte) at (0,2);
	\draw[name path=ellipse] (mitte) ellipse({\breite} and \hoehe);	
...
=======
	...
	\begin{scope}
		...
		\draw[dashed] (cntrr) ellipse({\kbreite} and {\khoehe});
	\end{scope}
	\path[name path=hor] (-1,0.5) -- ++(2,0);
	\path[name intersections={of=hor and ellipse}];
	\begin{scope}
		\clip (-1,0.5) rectangle ++(2,0.25);
		\draw[dashed] (0,0.5)  let \p1 = ($(intersection-1) - (intersection-2)$) in ellipse[x radius={0.5 * veclen(\x1,\y1)},y radius=0.125cm];
	\end{scope}
	\begin{scope}
		\clip (-1,0.5) rectangle ++(2,-0.25);
		\draw (0,0.5)  let \p1 = ($(intersection-1) - (intersection-2)$) in ellipse[x radius={0.5 * veclen(\x1,\y1)},y radius=0.125cm];
	\end{scope}
\end{tikzpicture}\end{center}
