...
\begin{emptythm}[Beweisskizze (zum Satz von Alexandrov-Toponogov \ref{thm:satz-alexandrov-toponogov})]
  ...
  Für $t \to t_0$ konvergiert $\delta(a,b_t,c_t)$ gegen ein Dreieck mit Umfang $\frac{2\pi}{\sqrt{\kappa}}$, d.h. das Vergleichsdreieck konvergiert gegen einen Großkreis.
  ...
  Es sei $c_s$ die minimierende, nach Bogenlänge parametrisierte Geodätische von $p$ nach $c(s)$ und $Y$ das von $c_s$ erzeugte Varationsfeld.
  Dann gilt
  \begin{align*}
...
% Satz 9.18
\begin{Satz}[Gronov]\index{Gronov}
  Es existieren Konstanten $c(n), c(n,\lambda,D)$, so dass gilt:
  \begin{enumerate}[label=(\roman*)]
  \item Hat $M^n$ $\sec \geq 0$, so läßt sich $\pi_1(M^n)$ mit $\leq c(n)$ Elementen erzeugen.
  \item Hat $M^n$ $\sec \geq - \lambda^2$ und $\diam(M) \leq D$, so lässt sich $\pi_1(M)$ mit $\leq c(n,\lambda,D)$ 
  \end{enumerate}
\end{Satz}
...
\begin{emptythm}[Beweisskizze]
  ...
  \begin{center}
    \textcolor{red}{Abbildung: Überlagerung}
  \end{center}
  Für $\gamma \in \Gamma$ sie $|\gamma| = \dop(x_0, \gamma \cdot x_0)$.
  ...
  $\#\{\gamma \in \Gamma \mid | \gamma | \leq r \} < \infty$, denn andernfalls gäbe es eine (nichtkonstante) Folge $\gamma_i \in M$ mit $|\gamma_i| \leq r$, d.h. 
  \begin{align*}
  ...
  \end{align*}
  Wähle $\gamma_1 \in \Gamma$ mit $|\gamma_1| = \min \{ |\gamma| \mid \gamma \in \Gamma \setminus \{1_p\} \}$.
  Sind $\gamma_1, \ldots, \gamma_k$ gewählt mit $|\gamma_1| \leq |\gamma_2| \leq \cdots \leq |\gamma_k|$, so bezeichne $G_k = \left< \gamma_1, \ldots, \gamma_k\right>$.
  ...
  \end{align*}
  Wäre $l_{ij} < l_j = |\gamma_j|$, so gälte für
  \begin{align*}
  ...
  \end{center}
  $r = \sin \frac{\pi}{6} = \frac{1}{2}$, also sind die Bälle $B_{\frac{1}{2}}(v_i)$ disjunkt.
  Betrachte das Volumen der \quot{unteren Hälfte}, diese liegt in $B_{\frac{\sqrt{5}}{2}}(0)$.
  Damit gilt
  \begin{align*}
  ...
  $\overline \alpha_{ij}$ fällt monoton in $l_i$, so wächst $\cos \overline \alpha_{ij}$ monotin in $l_i$ und es lässt sich folge Abschätzung verwenden:
  \begin{align*}
  ...
  \end{align*}
  Jedes $\gamma \in \pi_1(M)$ ist Produkt von Klassen von Schleifen der Länge $\leq 2D + \epsilon$.
  \begin{center}
    \textcolor{red}{Abbildung: Schleife mit Geodätischen an die Ecken}
  \end{center}
