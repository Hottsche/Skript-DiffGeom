...
\begin{bem}\begin{enumerate}[label=(\arabic*),leftmargin=*]
  ...
  \item
    Die Durchmesserschranke im Satz von Bonnet-Myers ist scharf in dem Sinne, dass falls $(M,g)$ eine vollständige Riemannsche Mannigfaltigkeit mit $\ric \geq (m-1)\frac{1}{r^2}$ ist und $\diam(M,g) = \pi r$ gilt, so folgt $(M,g)$ ist isometrisch zu $S^m_r$. (Cheng, 1975 \cite{Cheng1975})\index{Cheng}
  \end{enumerate}\end{bem}
...
\end{itemize}
Für $\sigma(s) = c_s(r)$ stimmt dann $\calJ$ mit obiger Konstruktion "uberein.
\begin{align*}
...
Es bezeichnen $\sn_\kappa$ und $\cs_\kappa$ die Lösungen mit $\sn_\kappa(0) = 0$, $\sn_\kappa'(0) = 1$ und $\cs_\kappa(0) = 1$, $\cs_\kappa'(0) = 0$, das hei"st
\begin{align*}
  &\sn_\kappa(t) = \begin{cases}
    \frac{1}{\sqrt{-\kappa}} \sinh(\sqrt{-\kappa}t) & \text{f"ur } \kappa < 0\\
    t & \text{f"ur } \kappa = 0\\
    \frac{1}{\sqrt{\kappa}} \sin(\sqrt{\kappa}t) & \text{f"ur } \kappa > 0\\
  \end{cases}\\
...
\end{align*}
das hei"st $b$ löst die eindimensionale Riccatigleichung.
Es sei $\calJ$ ein Jacobifeld entlang einer Geodätischen $c$ in $M^n_{\kappa}$ mit $\calJ(0) = 0$ und $\calJ \perp \dot c$.
...
\begin{align*}
  ...
  = \ct_{\kappa}Y.
\end{align*}
\emph{Ziel:} Wir suchen eine Abschätzung für Eigenwerte, beziehungsweise Operatornorm von $A_t$ und für das Wachstum von Jacobifeldern bei unteren Krümmungsschranken.

% Lemma 9.16
\begin{Lemma}\label{thm:lemma-9-16}
  Es seien $a,b \colon \calI \to \R$ glatt für ein Interval $\calI$, sowie
  \begin{itemize}
  \item $a' \leq -\kappa - a^2$
  \item $b' = -\kappa - b^2$
  \item $a(t_0) \leq b(t_0)$ für ein $t_0 \in \calI$.
  \end{itemize}
  Dann gilt:
  \begin{enumerate}[label=(\roman*)]
  \item $a(t) \leq b(t)$ f"ur alle $t \ge t_0$
  \item Gilt $a(t_0) = b(t_0)$ und $a(t_1) = b(t_1)$ für $t_1 > t_0$, so folgt $a|_{[t_0,t_1]} = b|_{[t_0,t_1]}$.
...
Es gelte $\sec_{(M,g)} \geq \kappa$ und es sei $c$ eine nach Bogenlänge parametrisierte Geodätische mit $c(0) = p$.
Wie oben bezeichne $A_t$ das symmetrische Endomorphismenfeld $A_t \colon \dot c(t) ^{\perp} \to \dot c(t)^{\perp}$, $X \mapsto \nabla_X \grad \dop_p$.
Für ein entlang $c$ paralleles Vektorfeld $E$ mit $\|E\| = 1$ und $E \perp \dot c$ sei $a = \left<AE,E\right>$.
Dann gilt:
\begin{align*}
  a' = \left<A'E,E\right> 
...
\begin{dfn}
  Ein \CmMark[Dreieck!geodätisches]{(geodätisches) Dreieck} $\Delta(a,b,c)$ in $M$ besteht aus drei geodätischen Segmenten $a, b$ und $c$ mit $a(0) = b(0), a(1) = c(0)$ und $c(1) = b(1)$.
  Es bezeichnen $\alpha, \beta, \gamma$ die inneren Winkel in $\Delta$.

  Ferner sei $|a| = \mathcal L(a)$ die Länge von $a$ (analog für $b$ und $c$). 
\end{dfn}

Ein Vergleichsdreieck in $M^2_{\kappa}$ der vollständigen Riemannschen Mannigfaltigkeit mit $\sec \cong \kappa$, ist ein Dreieck $\overline \Delta(\overline a,\overline b, \overline c)$ mit $|\overline a| = |a|$, $|\overline b| = |b|$ und $|\overline c| = |c|$.
Es existiert ein (bis auf Isomorphie) eindeutiges Vergleichsdreieck, wenn gilt:
\begin{enumerate}[label=(\roman*)]
\item $|a| + |b| \geq \kappa, |b| + |c| \geq |a|, |c| + |a| \geq |b|$
\item $\kappa(\Delta(a,b,c)) = |a| + |b| + |c| < \frac{2\pi}{\sqrt{\kappa}}$ (für $\kappa > 0$)
\end{enumerate}

\begin{center}
  \textcolor{red}{Abbildung: geodätische Dreiecke auf $S^2$}
\end{center}

\begin{Satz}[Alexander-Toponogov]\index{Alexander-Toponogov}\label{thm:satz-alexander-toponogov}
  Es sei $(M,g)$ eine vollständige Riemannsche $m$-Mannigfaltigkeit mit $\sec_{(M,g)} \geq \kappa$ und im Falle $\kappa > 0$ gelte $M \not\cong S^m_{\frac{1}{\sqrt{\kappa}}}$.
  Ist dann $\Delta(a,b,c)$ ein geodätisches Dreieck mit $|c| \leq |a| + |b|$ sowie, zum Falle $\kappa > 0$, $|c| < \frac{\pi}{\sqrt{\kappa}}$ und $a$ und $b$ seien minimierende Geodätische.
  Dann gilt für das Vergleichsdreieck $\overline\Delta(\overline a,\overline b, \overline c)$ in $M_{\kappa}$.
  \begin{enumerate}[label=(\roman*)]
...
\begin{center}
  \textcolor{red}{Abbildung: geodätisches Dreieck gegenüberliegende Seite}
\end{center}


\begin{bem}
  Das Aussschließen von $S^m_{\frac{1}{\sqrt{\kappa}}}$ ist nach dem Satz von Cheng keine Einschränkung.
\end{bem}

Es seien $a,b,c$ nach Bogenlänge parametrisiert und $p = a(0) = b(0)$.

\begin{center}
  \textcolor{red}{Abbildung: Eindeutige Geodätische von $p$ nach $c(t)$}
\end{center}

Es existiere zu jedem $c(t)$ eine eindeutige Geodätische von $p$.
Dann ist insbesondere $\dop_p$ um $c(t)$ glatt.
Die Abstandsfunktion wird nun so modifiziert, dass in dem Abschnitt nicht zwischen dem zu $\grad \dop_p$ kollinearen Anteil und dem zur Abstandssühäre tangentialen Teil unterschieden werden muss.
...
\begin{emptythm}[Karchers Trick]\index{Karchers Trick}
  ...
  \begin{align*}
    ...
    r'' & = \left<\nabla_{\dot c} \grad \dop_p,\dot c\right> = H_{\dop_p}(\dot c, \dot c).\cos
  \end{align*}
  ...
  \begin{align*}
    ...
    = \|w\|^2\left>A \frac{w}{\|w\|},\frac{w}{\|w\|}\right>
    \leq (\ct_{\kappa} \circ r) \|w\|^2.
  \end{align*}
  Somit gilt
  \begin{align*}
    c'' & = (m_{\kappa}''\circ r)r'^2 + (m_{\kappa}' \circ r) r''
  ...
  \begin{align*}
    \overline e = (\overline m_{\kappa} \circ \overline r), \
    \overline e '' + \kappa \overline e \equiv 1.
  \end{align*}
  Ist wie oben $\dot{\overline c} = \overline v + \overline w$ (mit $w,\overline w \neq 0$), so existiert ein Jacobifeld entlang der Geodätischen von $p$ nach $c(t)$ von der Form $J = \sn_{\kappa} E$, wobei $E$ die parallele Fortsetzung von $\overline w$ ist, und es gilt $J(\dop_{\overline p}(\overline c(t))) = w$.
  Damit folgt
  \begin{align*}
...
\begin{emptythm}[Beweisskizze]
  (zum Satz von Alexander-Toponogov \ref{thm:satz-alexander-toponogov})
  Annahme: $|a| + |b| + |c| < \frac{2\pi}{\sqrt{\kappa}}$.

  Fall $|a| + |b| = |c|$, korrekt.

  Es gelte also $|a| + |b| > |c|$, dann ist jedenfalls $p \notin c$.
  Es seien $r = \dop_p \circ c$ und $e = m_{\kappa} \circ r$ wie oben, sowie $\overline r$ und $\overline c$ für das Vergleichsdreieck.

  Betrachte $f = e - \overline e$.
  Fall 1: zu jedem $c(t)$ existiert eine eindeutige minimierende Geodätische, damit ist $r$ und damit $f$ differenzierbar.
  Es gilt
  \begin{align*}
    f'' = e'' - \overline e''
    \leq 1 - \kappa e - (1-\kappa \overline e)
    = -\kappa f.
  \end{align*}
  Angenommen es gäbe ein $s \in (0,l)$ $l = |c|$ mit $f(s) < 0$.
  Es sei $\epsilon > 0$ so, dass gilt
  \begin{align*}
    l + \epsilon < \frac{\pi}{\sqrt{\kappa + \epsilon}}.
  \end{align*}

  Es sei $g(t) = \sn_{\kappa + \epsilon}(t + \epsilon)$ die, auf $[0,l]$ positiv, Lösung von $g'' +(\kappa + \epsilon)g = 0$.
  Betrachte $h = \frac{f}{g}$.
  Dann gilt $h(s) < 0$ und $h(0) = 0 = h(l)$ und somit nimmt $h$ eine negatives Minimum, etwa $t_0$, an.

  Es gilt also $h(t_0) < 0, h'(t_0) = 0$ und $h''(t_0) \geq 0$.
  Dann folgt
  \begin{align*}
    f'' + \kappa f & = g'' h + 2 g'h' + gh'' + \kappa g h
    = -(\kappa + \epsilon)gh + 2 g'h' + gh'' + \kappa g h\\
    & = \underbrace{-\epsilon g h}_{> 0} + \underbrace{2 g' h'}_{= 0} + \underbrace{gh''}_{\geq 0} 
    > 0
  \end{align*}
  im Widerspruch zu $f'' + \kappa f \leq 0$.

  \begin{center}
    \textcolor{red}{Abbildung: nicht Fall 1}
  \end{center}
\end{emptythm}
