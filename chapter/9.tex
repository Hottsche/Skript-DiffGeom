...
\begin{bew}[von Satz \ref{satz-9-8}]\begin{enumerate}[label=(\roman*),leftmargin=*,widest=ii]
  ...
  \item
    ...
    Erinnerung: Ist $c \in \Omega_{pq}$ eine Geodätische und $t_o \in (0,1)$, so dass $c(t_0)$ zu $p = c(0)$ entlang $c$ konjugiert ist, so existiert ein Vektorfeld $Y$ entlang $c$ mit $\calI(Y,Y) < 0$.

    \begin{center}
      \textcolor{red}{Abbildung 1: Wdh: Sphäre}
    \end{center}

    Nun zum Beweis der Existenz von $Y$.

    Da $c(t_0)$ zu $p$ entlang $c$ konjugiert ist, existiert ein nichttriviales Jacobifeld $J$ entlang $c|_{[0,t_0]}$ mit $J(0) = 0, J(t_0) = 0$.
    Es sei $X$ das entlang $c$ parallele Vektorfeld mit
    \begin{align*}
      X(t_0) = -J'(t_0)
    \end{align*}
    ...
    \begin{align*}
      ...
      \begin{cases}
        ...
        \eta \cdot Z(t) & \text{ für } t_0 \leq t \leq 1
      \end{cases}
    \end{align*}
    $Y$ ist nun stückweise glatt, die Variantionsformel für $\mathcal L$ und $E$, bzw. die Indexform lassen sich aber ganz analog für stückweise glatte Vektorfelder bzw. Variationen formalisieren.
    Es gilt, da $Y$ orthogonales zu $c$ ist, für die zu $Y$ gehörigen Variationen $c_s$:
    \begin{align*}
      J(X,Y) = & \difffrac[s=0]{^2}{s^2}E(c_s) =
      \difffrac[s=0]{^2}{s^2} \mathcal L(c_s)\\
      = & \int_0^1\|Y'\| - \left<R(Y,\dot c)\dot c,Y\right>\\
    ...
    und mit 
    \begin{align*}
      & \left<J',J'\right> = \left<J',J'\right> + \left<J'',J\right> = \left<J',J'\right> - \left<R(J,\dot c)\dot c,J\right>\\
      & \left<J',Z\right>' = \left<J',Z'\right> + \left<J'',Z\right> = \left<J',Z'\right> - \left<R(J,\dot c) \dot c,Z\right>
    \end{align*}
    folgt
    \begin{align*}
    ...
    \end{align*}
    Für hinreichend kleines $\eta > 0$ ist damit $\calI(Y,Y) < 0$.
  \end{enumerate}\end{bew}

Betrachte die Sphäre vom Radius $r > 0$, $S^n_r \subset \R^{n+1}$:
...
\begin{align*}
  \ric_p^{S_r^n}(v,v) & = \sum_i \left<R(e_i,\frac{v}{\|v\|_1})\frac{v}{\|v\|_1},e_i\right>\\
...
\begin{bew}
  ...
  Für jedes Vektorfeld $Y$ entlang $c$, welches in den Endpunkten verschwinde, ist $J(Y,Y) \geq 0$.
...
\begin{bem}
  ...
  \item Die Existenz einer uniformen positiven Krümmungs ist entprechend
    \begin{align*}
  ...
  \item Die Durchmesserschranke im Satz von Bonnet-Myers ist scharf in dem Sinne, dass falls $(M,g)$ vollständig Riemannsche Mannigfaltigkeit mit $\ric \geq (m-1)\frac{1}{r^2}$ ist und $\diam(M,g) = \pi r$ gilt, so folgt $(M,g)$ ist isometrisch zu $S^m_r$. (Cheng, 1975)
  \end{enumerate}
\end{bem}
...
\begin{bem}
  ...
  \item Sind zwei glatte Wege homotop, so kann eine glatte Homotopie gewählt werden.
    Die \CmMark{Fundamentalgruppe} $\pi_1(M,p)$ ist die Menge der Homotopieklassen von Wegen $c \in \Omega_{pp}$ bezüglich eigentlicher Homotopien mit der durch die Verkettung von Wegen induzierten Gruppenstruktur.
    \begin{center}
